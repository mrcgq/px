...
func (s *Stream) Close() {
	if !atomic.CompareAndSwapInt32(&s.State, int32(StateConnected), int32(StateClosed)) &&
		!atomic.CompareAndSwapInt32(&s.State, int32(StateConnecting), int32(StateClosed)) &&
		!atomic.CompareAndSwapInt32(&s.State, int32(StateInit), int32(StateClosed)) {
		return
	}
